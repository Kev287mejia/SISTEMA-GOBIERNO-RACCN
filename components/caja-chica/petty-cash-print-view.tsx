...
export function PettyCashPrintView({ movement }: { movement: any }) {
    useEffect(() => {
    ...
    return (
        <div className="bg-white p-8 max-w-[21cm] mx-auto min-h-screen">
            ...
            <div className="flex justify-between items-start mb-8 border-b-2 border-slate-800 pb-6">
                <div className="flex gap-4 items-center">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-600 font-medium">Departamento de Tesorería</p>
                        <p className="text-xs text-slate-500">Control de Caja Chica</p>
                    </div>
                </div>
                <div className="text-right">
                    <div className="inline-block bg-slate-100 px-4 py-2 rounded-lg border border-slate-200">
                        <p className="text-xs text-slate-500 uppercase font-bold tracking-wider">Vale de Caja</p>
                        <p className="text-2xl font-black text-slate-900">
                            {movement.referencia || 'S/N'}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <DateDisplay date={movement.fecha} />
                        </div>
                        <div>
                ...
                <div className="bg-slate-50 p-6 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center mb-4 border-b border-slate-200 pb-4">
                        ...
                        <span className="text-2xl font-black text-slate-900">{formatCurrency(movement.monto)}</span>
                    </div>
                    <div className="space-y-2">
                        <div className="flex justify-between text-xs">
                            <span className="text-slate-500">Caja Chica Origen</span>
                            <span className="font-mono font-bold text-slate-700">{movement.pettyCash?.nombre || 'General'}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                            <span className="text-slate-500">Solicitante</span>
                            <span className="font-medium text-slate-700">{movement.usuario?.nombre} {movement.usuario?.apellido}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {/* Evidencia Section */}
            {movement.evidenciaUrls && movement.evidenciaUrls.length > 0 && (
                <div className="mt-8 page-break-inside-avoid">
                    <h3 className="text-xs font-black text-slate-900 uppercase border-b border-slate-200 pb-2 mb-4 flex items-center gap-2">
                        <span>Soporte Digital Adjunto</span>
                        <span className="bg-slate-100 text-slate-500 px-1.5 rounded text-[10px]">{movement.evidenciaUrls.length}</span>
                    </h3>
                    <div className="grid grid-cols-2 gap-4">
                        {movement.evidenciaUrls.map((url: string, idx: number) => {
            ...
            <div className="mt-16 pt-8 border-t border-slate-200 grid grid-cols-3 gap-8 text-center text-xs page-break-inside-avoid">
                <div className="pt-8 border-t border-slate-400 mx-4">
                    <p className="font-bold text-slate-900 mb-1 uppercase">{movement.usuario ? `${movement.usuario.nombre} ${movement.usuario.apellido}` : 'Firma Pendiente'}</p>
                    <p className="text-slate-500">Solicitante / Recibí Conforme</p>
                </div>
                <div className="pt-8 border-t border-slate-400 mx-4">
                    <p className="font-bold text-slate-900 mb-1 uppercase">Carlos Alemán</p>
                    <p className="text-slate-500">Coordinador de Gobierno / Vo.Bo.</p>
                </div>
                <div className="pt-8 border-t border-slate-400 mx-4">
            ...
            <div className="mt-12 text-[9px] text-slate-400 text-center font-mono uppercase">
                Documentame generado por SISTEMA GOBIERNO
            </div>
        </div>
    )
}
