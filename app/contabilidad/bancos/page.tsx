...
export default function CuentasBancariasPage() {
    ...
    const aggregatedTotals = useMemo(() => {
        return (accounts || []).reduce((acc, curr) => ({
            bankTotal: acc.bankTotal + (Number(curr.bankBalance) || 0),
            floatingTotal: acc.floatingTotal + (Number(curr.floatingWithdrawals) || 0),
            projectedTotal: acc.projectedTotal + (Number(curr.projectedBalance) || 0),
        ...
        <DashboardLayout>
            <div className="min-h-screen bg-[#fcfcfc] pb-20">
                <ModuleHero 
                    ...
                    subtitle="CONTROL DE LIQUIDEZ, CONCILIACIÓN BANCARIA Y DISPONIBILIDAD REAL"
                />

                <div className="max-w-6xl mx-auto px-8 -mt-20 relative z-30 space-y-10">
                    {!loading && accounts.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <Card className="border-none shadow-xl bg-white overflow-hidden group">
                                ...
                                <CardContent>
                                    <div className="text-3xl font-black text-slate-900">{formatCurrency(aggregatedTotals.bankTotal)}</div>
                            ...
                            <Card className="border-none shadow-xl bg-white overflow-hidden group">
                                ...
                                <CardContent>
                                    <div className="text-3xl font-black text-amber-600">-{formatCurrency(aggregatedTotals.floatingTotal)}</div>
                            ...
                            <Card className="border-none shadow-xl bg-indigo-950 text-white overflow-hidden relative group">
                                ...
                                <CardContent>
                                    <div className="text-2xl lg:text-3xl font-black text-white tracking-tighter truncate" title={formatCurrency(aggregatedTotals.projectedTotal)}>
                                        {formatCurrency(aggregatedTotals.projectedTotal)}
                                    </div>
